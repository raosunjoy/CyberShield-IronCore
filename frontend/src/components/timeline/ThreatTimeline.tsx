...
const ThreatTimeline: React.FC<ThreatTimelineProps> = ({
  ...
  const [filter, setFilter] = useState<
    'all' | 'detection' | 'analysis' | 'mitigation'
  >('all');
    ...
    <div className='bg-black border border-green-400 font-mono'>
      ...
      <div className='p-4 border-b border-green-400 bg-gray-900'>
        ...
        <div className='flex gap-4 mt-3 text-xs'>
          <div>
            Total Events:{' '}
            <span className='text-yellow-400'>{events.length}</span>
          </div>
          <div>
            AI Decisions:{' '}
          ...
          <div>
            ...
            <span className='text-purple-400'>
              {events.length > 0 && events[0]
                ? formatTimeElapsed(events[0].timestamp)
                : 'N/A'}
            </span>
          </div>
        </div>
                ...
                <div className='flex justify-between items-start mb-2'>
                  <div className='flex-1'>
                    <div className='flex items-center gap-2 mb-1'>
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${getActorColor(event.actor)} bg-black bg-opacity-50`}
                      >
                        {event.actor}
                      </span>
                  ...
                  <div className='mt-2'>
                    ...
                    <ul className='text-xs text-gray-300'>
                      {event.details.actions.map((action, actionIndex) => (
                        <li key={actionIndex} className='ml-2'>
                          • {action}
                        </li>
                      ))}
                    </ul>
            ...
            >
                ...
                <div className='flex justify-between items-center'>
                  <div>
                    ...
                    <div className='flex items-center gap-2 mt-1'>
                      <span
                        className={`text-sm ${getActorColor(selectedEvent.actor)}`}
                      >
                        {selectedEvent.actor}
                      </span>
              ...
              <div className='p-4'>
                <div className='space-y-4'>
                  ...
                  {selectedEvent.details.evidence &&
                    selectedEvent.details.evidence.length > 0 && (
                      <div>
                        <h5 className='text-green-400 font-bold mb-2'>
                          Evidence
                        </h5>
                  ...
                  {selectedEvent.details.actions &&
                    selectedEvent.details.actions.length > 0 && (
                      <div>
                        <h5 className='text-green-400 font-bold mb-2'>
                          Actions Taken
                        </h5>
                        <ul className='text-gray-300 space-y-1'>
                          {selectedEvent.details.actions.map(
                            (action, index) => (
                              <li
                                key={index}
                  ...
                  {selectedEvent.actor === 'AI' &&
                    explainAIDecision(selectedEvent)}
