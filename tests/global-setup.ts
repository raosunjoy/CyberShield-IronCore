...
export default async function globalSetup(): Promise<void> {
  console.log('🛡️  CyberShield-IronCore: Initializing Enterprise Test Environment');
  ...
  try {
    ...
    process.env['DATABASE_URL'] = 'postgresql://test:test@localhost:5432/cybershield_test';
    
    console.log('✅ Test database configured');
  } catch (error) {
    console.warn('⚠️  Test database not available, skipping database tests', error);
  }
