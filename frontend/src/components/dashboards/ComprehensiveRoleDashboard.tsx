...
export default function ComprehensiveRoleDashboard({
  ...
  const getIncidentResponderWidgets = (): DashboardWidget[] => [
    ...
    },
    {
      ...
      data: [
        ...
        {
          incident: 'INC-2024-0845',
          type: 'Ransomware',
          status: 'Recovery',
          phase: 'System Restoration',
          eta: '4h',
        },
        {
          incident: 'INC-2024-0843',
          type: 'Insider Threat',
  ...
  const getComplianceWidgets = (role: UserRole): DashboardWidget[] => [
    ...
    },
    {
      id: 'compliance-tools',
      title:
        role === UserRole.AUDITOR ? '🔎 Audit Tools' : '📋 Compliance Tools',
      type: 'action',
      size: 'medium',
      visible: true,
      data:
        role === UserRole.AUDITOR
          ...
          : [
              ...
              {
                action: 'Assessment Tools',
                icon: '✅',
                href: '/compliance/assess',
              },
              {
                action: 'Training Tracker',
                icon: '🎓',
                href: '/compliance/training',
              },
              {
                action: 'Regulatory Reports',
                icon: '📊',
    ...
    },
    {
      ...
      data: [
        ...
        },
        {
          item: 'Public Reports',
          access: 'Available',
          description: 'Access published security reports',
        },
        {
  ...
  const renderWidget = (widget: DashboardWidget) => {
    ...
    if (
      ...
      !hasPermission(currentRole, widget.requiredPermission)
    ) {
      return null;
    }
    if (
      widget.requiredFeature &&
      !hasFeature(currentRole, widget.requiredFeature)
    ) {
      return null;
    }
    if (!widget.visible) {
          ...
          <div key={widget.id} className={baseClasses}>
            ...
            <div className='space-y-4'>
              {widget.data.map((item: any, index: number) => (
                <div
                  key={index}
                  className='flex justify-between items-center p-3 border border-green-400/20 rounded'
                >
                  <div>
                    <div className='text-green-400 font-medium'>
                      {item.tool || item.system}
                    </div>
                    <div className='text-green-400/70 text-sm'>
                      {item.uptime || item.throughput}
                    </div>
                  </div>
                  <div
      ...
      <div className='min-h-screen bg-black text-green-400 font-mono flex items-center justify-center'>
        <div className='text-center'>
          ...
          <h1 className='text-3xl font-bold mb-4'>
            Loading {roleConfig.displayName} Dashboard...
          </h1>
    ...
    <div className='min-h-screen bg-black text-green-400 font-mono'>
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        <div className='mb-8'>
          ...
          <div className='flex items-center justify-between'>
            <p className='text-green-400/70 text-lg'>
              {roleConfig.level.charAt(0).toUpperCase() +
                roleConfig.level.slice(1)}
              -level security dashboard with {roleConfig.features.length}{' '}
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          {dashboardData?.widgets?.map((widget: DashboardWidget) =>
            renderWidget(widget)
          )}
        </div>
