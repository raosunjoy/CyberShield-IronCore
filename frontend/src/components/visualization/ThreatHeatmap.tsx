  ...
  const drawHeatmap = () => {
    ...
    threatData.forEach(threat => {
      ...
      gradient.addColorStop(
        0,
        getSeverityColor(threat.severity, threat.riskScore)
      );
      gradient.addColorStop(
        0.5,
        getSeverityColor(threat.severity, threat.riskScore * 0.5)
      );
    ...
    <div className='relative'>
          ...
          <div className='space-y-1'>
            ...
            <div>
              ...
              <span className='text-blue-400'>
                {Math.round(hoveredThreat.aiConfidence * 100)}%
              </span>
            </div>
            <div>
              Severity:{' '}
              <span
                className={
                  hoveredThreat.severity === 'critical'
                    ? 'text-red-500'
                    : hoveredThreat.severity === 'high'
                      ? 'text-orange-500'
                      : hoveredThreat.severity === 'medium'
                        ? 'text-yellow-500'
                        : 'text-green-500'
                }
              >
      ...
      <div className='absolute top-4 left-4 bg-black bg-opacity-80 border border-green-400 p-3 font-mono text-xs'>
        ...
        <div className='space-y-1'>
          <div>
            Total Threats:{' '}
            <span className='text-yellow-400'>{threatData.length}</span>
          </div>
          <div>
            Critical:{' '}
          ...
          <div>
            ...
            <span className='text-blue-400'>
              {threatData.length > 0
                ? Math.round(
                    threatData.reduce((sum, t) => sum + t.riskScore, 0) /
                      threatData.length
                  )
                : 0}
            </span>
