...
describe('JarvisAssistant', () => {
  ...
  it('renders JARVIS assistant with proper title', () => {
    ...
    expect(
      screen.getByText('Just A Rather Very Intelligent System')
    ).toBeInTheDocument();
  });
  ...
  it('shows welcome message when no messages exist', () => {
    ...
    expect(
      screen.getByText("Hello! I'm JARVIS, your AI security assistant.")
    ).toBeInTheDocument();
    expect(
      screen.getByText('Try saying "show threats" or "system status"')
    ).toBeInTheDocument();
  });
  ...
  it('processes "show threats" command correctly', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/Displaying current threat dashboard/)
        ).toBeInTheDocument();
  ...
  it('processes "system status" command correctly', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/System status: All systems operational/)
        ).toBeInTheDocument();
  ...
  it('processes "analyze threat" command with threat ID', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/Analyzing threat T-2024-005/)
        ).toBeInTheDocument();
  ...
  it('handles unknown commands gracefully', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/I understand you're asking about/)
        ).toBeInTheDocument();
        expect(
          screen.getByText(/Here are some commands I can help with/)
        ).toBeInTheDocument();
      },
      { timeout: 2000 }
    );
  ...
  it('processes "block ip" command and sends WebSocket message', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/IP address 192.168.1.100 has been blocked/)
        ).toBeInTheDocument();
      },
      { timeout: 2000 }
    );
  ...
  it('renders in minimized state when prop is provided', () => {
    ...
    render(
      <JarvisAssistant isMinimized={true} onToggleMinimize={mockToggle} />
    );
  ...
  it('processes risk assessment command', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/Risk assessment complete/)
        ).toBeInTheDocument();
        expect(
          screen.getByText(/15 vulnerabilities identified/)
        ).toBeInTheDocument();
      },
      { timeout: 2000 }
    );
