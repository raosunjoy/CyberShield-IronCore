...
export default function RoleBasedDashboards() {
  ...
  const [currentRole, setCurrentRole] = useState<UserRole>(UserRole.SECURITY_MANAGER);
  const [showRoleSelector, setShowRoleSelector] = useState(false);
  const [loading, setLoading] = useState(true);

  // Generate available roles from RBAC configuration
  const availableRoles: RoleOption[] = Object.entries(ROLE_CONFIGURATIONS).map(([key, config]) => ({
    id: key,
    ...
    return (
      <div className="min-h-screen bg-black text-green-400 font-mono flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold mb-4">Loading Role-Based Dashboard...</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            ...
            <p className="text-green-400/70 text-lg">
              Choose a role to experience the tailored dashboard view (Demo: All {availableRoles.length} enterprise roles)
            </p>
          </div>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-bold ${
                      config.level === 'executive' ? 'bg-purple-400/20 text-purple-400' :
                      config.level === 'manager' ? 'bg-blue-400/20 text-blue-400' :
                      config.level === 'analyst' || config.level === 'specialist' ? 'bg-green-400/20 text-green-400' :
                      config.level === 'user' ? 'bg-yellow-400/20 text-yellow-400' :
